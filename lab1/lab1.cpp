...
int main() //Controlos the program flow, handles user input, calculates distances and reapeats based on user choice.
{
	...
	do {// The loop runs at least once, then repeats if 'again' is 'Y' or 'y', otherwise it exits the loop. 
		...
		if (choice == 'E' || choice == 'e') {// The statement is checking if the variable "choice" is either uppecase or lowercase. 
			// Call getValidadtedInput() to ensure x1/y1/x2/y2 are between -10 and 10.
			...
			//Generate random x1/y1/x2/y2 between -10 and 10
			x1 = randomCoordinate();
			y1 = randomCoordinate();
			x2 = randomCoordinate();
			y2 = randomCoordinate();
		...
		if (dist1 < dist2) // If point 1 is closer to the origen. 
			cout << " Point 1 is closer to the origin.\n";
		else if (dist2 < dist1) // If point 2 is closer to the origen.
